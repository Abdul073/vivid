...
const ThemePicker = ({ onThemeSelect, selectedTheme, themes }: Props) => {
  ...
  const handleGeneareTheme = async () => {
    setLoading(true);
    if (!selectedTheme) {
      toast.error("Error", {
  ...
  return (
    <div
      className="w-[400px] sticky top-0 h-screen flex flex-col shadow-lg"
      style={{
        backgroundColor:
    ...
    >
      <div className="p-8 space-y-6 flex-shrink-0">
        <div className="space-y-2">
          ...
          >
            Pick a <span className="text-vivid">theme</span>
          </h2>
          <p
            className="text-sm"
            style={{ color: `${selectedTheme.accentColor}80` }}
          >
            Choose from our current collection or generate a custom theme
          </p>
        </div>
        <Button
